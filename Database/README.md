...
## DB 키
- 키: 튜플들을 서로 구분할 수 있는 기준이 되는 애트리뷰트
- 종류
  1. 후보키: 릴레이션을 구성하는 속성들 중 튜플을 유일하게 식별 가능한 속성들의 부분집합, 즉 기본키로 사용 가능한 속성들, 유일성 및 최소성을 만족
  2. 기본키: 후보키 중 선택한 주 키, not null
  3. 대체키: 후보 키가 둘 이상일 때 기본키를 제외한 나머지 후보키
  4. 슈퍼키: 유일성은 만족시키지만 최소성은 만족시키지 않는 키

## DB 정규화
### 이상 현상
- Anomly
- 삭제/삽입/수정 이상 존재
  + 삭제 이상: 튜플 삭제 시 다른 정보까지 삭제
  + 삽입 이상: 삽입 시 특정 속성에 해당 값이 없어 NULL을 입력해야하는 상황
  + 수정 이상: 튜플 수정 시 일부분만 수정이 일어나는 데이터 불일치 현상

### 정규화
- 이상현상이 발생하는 릴레이션을 분해해서 이상 현상을 없애는 과정
- 무결성 유지 및 DB 저장 용량을 줄이는 효과
- 과정
  ![first](../image/db_first.png)
  1. 제1정규화: 테이블 컬럼이 원자값을 가지도록 테이블 분해

  ![second](../image/db_second.png)
  2. 제2정규화: 완전 함수 종속을 만족하도록 테이블 분해, 기본키의 부분집합이 결정자가 되면 안됨

  ![third](../image/db_third.png)
  3. 제3정규화: 이행적 종속(A->B->C 구조) 삭제

  ![bncf](../image/db_bncf.png)
  4. BNCF 정규화: 모든 결정자가 후보키가 되도록 테이블 분해  
