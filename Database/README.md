...
## 트랜잭션
### 정의
- **DB의 상태를 변환시키는 작업을 수행하기 위해 필요한 DB 연산들을 모아놓은 것**
...
- 장점
  + 항상 정렬된 상태를 유지해서 값을 탐색 속도가 빠름
  + 전반적인 시스템 부하 감소
- 단점
  + 별도의 저장공간 필요
  + Index 생성을 위한 추가적인 작업 필요
  + 생성/삭제/수정이 잦을 경우 성능이 오히려 저하
- Index를 사용하면 좋은 경우
  + 규모가 큰 테이블
  + 생성/삭제/수정이 빈번하지 않음
  + Join/Where/Order By 자주 사용할 시
  + 데이터 중복도가 낮은 컬럼

### Index 자료구조
#### Hash Table
- 자료구조에 대한 설명은 자료구조 참조
- 빠른 데이터 검색이 필요할 때 유용(탐색 시간: O(1))
- **부등호 연산이 자주 사용되는 데이터베이스에 부적합**

#### B-tree
![btree](../image/database_btree.png)

- Root, Branch, Leaf 노드들로 구성, key 및 data 저장
- 노드 하나에 **2개 이상**의 자식 노드 저장
- Balanced Tree, 균형이 깨질 시 이를 조정하는 작업 존재
- 높이가 일정하기 때문에 탐색 시 항상 O(logN)을 만족
- **노드가 배열 형태로 저장**되기 때문에 같은 노드끼린 포인터로 접근할 필요없이 바로 **다음 인덱스 접근을 허용**하므로 RBT보다 효율적.
...
- B-tree를 적절히 개선한 버전
- Leaf 노드(Data 노드)에서만 data 저장, 나머지 노드(Index 노드)엔 key만 저장
- Leaf 노드들을 **Linked List로 연결, 순차 검색을 용이**하게 함
- 다만 B-Tree에 비해 무조건 Leaf 노드까진 가야 한다는 단점 존재
