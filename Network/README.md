...
- 특징
1. Server-Client: 서버는 자원을 가지고 있고 클라이언트는 자원을 요청
2. Stateless: 각각의 요청은 다른 요청에 종속적이지 않고 독립적임
3. Cacheable: HTTP 프로토콜에서 대용량이 보내질 시 cache에서 확인해서 요청을 더 빠르게 처리 가능
4. Layered System: 서버는 다중 계층으로 구성 가능. 순수 비즈니스 로직을 수행하는 API 서버와 그 앞단에 사용자 인증, 암호화, 로드 밸런싱을 하는 계층을 추가해서 구조상의 유연성을 둘 수 있음
5. Self-descriptiveness(자체 표현 구조): API 메세지만 보고도 이해할 수 있는 자체 구조로 이뤄짐
6. Uniform: HTTP 표준에만 따른다면 다른 언어 등에 종속되지 않고 일관성을 유지.
