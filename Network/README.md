...
#### HTTPS 동작 과정
- 공개키 방식은 많은 컴퓨터 파워를 사용
- 대칭키는 보안에 취약
- 그러므로 **먼저 핸드셰이킹 과정에서 공개키 방식으로 대칭키 암호화, 실제 데이터를 주고받을 때 대칭키 사용**
...
1. Handshake  
  ...
  - 주고받는 정보
    ...
    + 클라이언트 지원 암호화 방식
    + 이미 핸드셰이킹을 했을 시 기존 세션 재활용할 세션 아이디

  2) Server Hello: 서버에서 응답  
  - 주고받는 정보
    + 서버 측에서 생성한 랜덤 데이터
    + 클라이언트에서 준 정보 중 서버가 선택한 클라이언트 암호화 방식
    + 인증서

  3) 인증서 확인
  - 클라이언트에 내장된 CA 리스트 확인
  - 확인 위해 CA 공개키를 이용해 인증서 복호화, 성공했다면 인증 성공
  - 서버/클라이언트 랜덤 데이터 조합해서 pre master secret 키 생성, 세션 단계에서 데이터 주고 받을 때 암호화 위한 **대칭 키**
  - pre master secret를 **공개키**로 암호화해서 서버로 전송

  4) 서버에서 키 받음
  - 클라이언트에서 전송한 pre master secret 값을 비공개키로 복호화
  - 서버와 클라이언트는 pre master secret 값을 master secret 값으로 교체
  - master secret는 session key값 생성, 이를 이용해 대칭키 암호화 실행

  5) 핸드쉐이크 단계 종료

2. 세셴
- 실제 서버와 클라이언트가 데이터를 주고 받는 단계
- **정보를 상대에게 전송하기 전에 session key값을 이용해 대칭키 방식으로 암호화**

3. 세션 종료
- 데이터 전송이 끝날 시 SSL 통신이 끝났음을 서로에게 알림
- 세션키 폐기
...
- 소켓
  ...
  + 클라이언트 및 서버 소켓을 통해 네트워크 연결을 만듦
  + 클라이언트에서 IP 주소 및 포트번호로 식별 대상에게 요청
  + 서버에서 어떤 연결 요청을 받아들일지 시스템에 등록

![socket](../image/network_socket.png)

- 처리 방법
  + 클라이언트
  1. 소켓 생성: socket()
    * 소켓 종류 지정(TCP-스트림 타입, UDP - 데이터그램 타입)
  2. 연결 요청: connect()
    * IP 주소 및 포트 번호로 식별되는 대상으로 연결 요청
    * 연결 요청에 대한 결과가 결정될 때까지 대기
  3. 데이터 송수신: send() / recv()
    * 두 호출 모두 결과가 결정되지 전까지 API 리턴 안함
    * recv()는 언제 올지 모르므로 별도의 스레드로 실행
  4. 소켓 닫기: close()
    * 송수신이 끝난후 소켓 닫기
    * 재사용 불가
  + 서버
  1. 소켓 생성: socket()
    * 소켓 생성
  2. 소켓 바인딩: bind()
    * 파라미터: 포트번호 or IP주소 + 포트 번호
    * 중복 포트 사용 시 에러 리턴
  3. 연결 요청 대기: listen()
    * 클라이언트 요청 대기
    * 요청이 수신되는 경우 및 에러가 발생하는 경우 탈출
    * 연결 정보는 내부 관리 큐에 쌓임
    * UDP에선 사용 안함
  4. 연결 수립: accept()
    * 소켓 간 연결 수립
  ...
  5. 데이터 송수신: send()/recv()
    * 클라와 동일
  6. 소켓 연결 종료: close()
    * 클라이언트 연결 수립 소켓 + 클라이언트 연결 대기 소켓을 닫아줌
- 출처: https://velog.io/@devsh/%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC-%EA%B8%B0%EC%B4%88-%EA%B0%9C%EB%85%90-%EC%A0%95%EB%A6%AC%ED%95%98%EA%B8%B0-%EC%86%8C%EC%BC%93%EA%B3%BC-%EC%86%8C%EC%BC%93-%ED%94%84%EB%A1%9C%EA%B7%B8%EB%9E%98%EB%B0%8D-%EA%B0%9C%EB%85%90
