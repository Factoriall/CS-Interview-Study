...
1. 공유 메모리: 다수의 프로세스에 의해 공유되는 메모리 영역
  + OS의 관여가 없어 속도가 빠름
  + 보안 및 동기화 문제
2. 메세지 전달: 프로세스 사이 데이터 송수신 또는 동기화 작업
  + 분산 시스템 환경에서 주로 사용
  + 데이터 교환 개수가 적을 때 유용
...
- Stack을 독립적으로 가지는 이유: 독립적인 함수 호출을 통해 **독립적인 실행 흐름 추가**를 위해
...
- 페이지는 **페이저**로 관리, 실제 필요한 페이지들만 메모리로 읽어 옮으로서 사용되지 않을 페이지를 가져오는 시간 낭비/메모리 낭비를 줄일 수 있음, 페이저는 캐시이기도 함.
